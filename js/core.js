...
autoSendInterval: 5,
        ...
        // 【新增】拼接字卡默认设置
        stitchReplyEnabled: false,
        stitchReplyProbability: 30,
        stitchReplyMin: 2,
        stitchReplyMax: 4
            };
        }
...
const loadData = async () => {
    try {
        ...
        // 【新增】拼接字卡默认值保护
        if (settings.stitchReplyProbability === undefined) settings.stitchReplyProbability = 30;
        if (settings.stitchReplyMin === undefined) settings.stitchReplyMin = 2;
        if (settings.stitchReplyMax === undefined) settings.stitchReplyMax = 4;
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            let stitchCount = 2;
            if (settings.stitchReplyEnabled && replyPoolOnce.length >= 2) {
                const prob = Math.max(10, Math.min(50, settings.stitchReplyProbability || 30));
                if (Math.random() * 100 < prob) {
                    shouldStitch = true;
                    const minCount = Math.max(2, Math.min(5, settings.stitchReplyMin || 2));
                    const maxCount = Math.max(minCount, Math.min(5, settings.stitchReplyMax || 4));
                    stitchCount = minCount + Math.floor(Math.random() * (maxCount - minCount + 1));
                }
            }
                        ...
                        for (let s = 0; s < stitchCount; s++) {
                            ...
                            for (let t = 0; t < 10; t++) {
                                ...
                                if (picked && String(picked).trim()) {
                                    part = String(picked).trim();
                                    break;
                                }
                            }
                            if (part) stitchedParts.push(part);
                        }
                        if (stitchedParts.length > 0) {
                            replyText = stitchedParts.join(' ');
                        } else {
                            shouldStitch = false;
                        }
                    }
    ...
    document.getElementById('welcome-subtitle-scramble').textContent = "请在设置中添加开场动画";
        ...
        const updateUI = () => {
            ...
            const _pillSyncMap = {
                ...
                // 【新增】拼接字卡开关同步
                '#stitch-reply-toggle': 'stitchReplyEnabled'
            };
